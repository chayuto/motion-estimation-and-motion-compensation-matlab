...
for  m=StartFrame+1:StopFrame % loop for all frame-pair
    ...
    legend('FD','DFD(3)','DFD(7)','DFD(11)','Location','SouthEast');
    title('MSE');
    hold on 
   ...
   strText = '3,7 and 11 shift';
   RGB = insertText( GridOut, [10,10],strText, 'FontSize', 18, 'FontSize', 18);
   figure(2);
   imshow(RGB);
   
   imwrite(RGB,strcat('MC2_',RNAME,num2str(iBlkSize),'_',num2str(m),'.bmp'),'bmp');
  
   [img,idx] = rgb2ind(RGB,255);
   if m == StartFrame+1;
		imwrite(img,idx,strcat('MC2_',RNAME,num2str(iBlkSize),'.gif'),'gif','LoopCount',Inf,'DelayTime',0.13);
   else
		imwrite(img,idx,strcat('MC2_',RNAME,num2str(iBlkSize),'.gif'),'gif','WriteMode','append','DelayTime',0.13);
   end
