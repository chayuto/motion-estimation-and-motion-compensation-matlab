...
for  m=StartFrame+1:StopFrame-1 % loop for all frame-pair
   ...
   if m == StartFrame+1;
		imwrite(A,map,strcat('BM_',RNAME,num2str(iBlkSize),'_''.gif'),'gif','LoopCount',Inf,'DelayTime',0.12);
   else
		imwrite(A,map,strcat('BM_',RNAME,num2str(iBlkSize),'_''.gif'),'gif','WriteMode','append','DelayTime',0.12);
   end
